...
export default function BookmarkWidget() {
  ...
  return (
    <div className="bg-th-surface rounded-lg p-4">
      ...
      {bookmarks.length === 0 ? (
        <p className="text-th-dim text-xs text-center py-4">No bookmarks yet</p>
      ) : (
          ...
          {bookmarks.slice(0, 5).map(item => (
            <div key={item.id} className="flex items-center gap-2 group">
              ...
              <div className="flex-1 min-w-0">
                <Link to={`/manga/${item.slug}`} className="text-xs text-th-text group-hover:text-[#366ad3] transition-colors line-clamp-1 block">
                  {item.title}
                </Link>
              ...
              <button
                onClick={() => toggle(item)}
                className="text-th-dim hover:text-red-400 transition-colors text-xs shrink-0"
                title="Remove"
              >
                ✕
              </button>
