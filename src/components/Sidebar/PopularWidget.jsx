          ...
          <button
            ...
            className={`flex-1 text-xs py-1 rounded capitalize transition-colors ${
              activeTab === tab
                ...
                : 'bg-[#222] text-[#888] hover:text-[#ddd]'
            }`}
          >
        ...
        {items.map(item => (
          ...
          >
            <span className={`w-6 h-6 flex items-center justify-center text-xs font-bold rounded shrink-0 ${
              item.rank <= 3 ? 'bg-[#366ad3] text-white' : 'bg-[#222] text-[#888]'
            }`}>
              {item.rank}
            </span>
            <img src={item.cover} alt={item.title} className="w-8 h-10 object-cover rounded shrink-0" loading="lazy" />
            <div className="flex-1 min-w-0">
              <p className="text-xs text-[#ddd] group-hover:text-[#366ad3] transition-colors line-clamp-1 font-medium">
                {item.title}
              </p>
              <p className="text-[10px] text-[#555] flex items-center gap-0.5">
